...
def FindPeak(dataSet):
    ...
    for i in range(hist.GetNbinsX()-1, 0, -1): # Step from right to left.
        ...
        if hist.GetBinContent(i + 1) <= 0.3*hist.GetMaximum(): continue
        #if hist.GetBinContent(i+1) <= 0: continue
        sigma = math.sqrt(hist.GetBinContent(i) + hist.GetBinContent(i+1))
        if (hist.GetBinContent(i+1) - hist.GetBinContent(i))/sigma >= 4.: # 3.:
...
def DoFit(dataSet, PeakPosGuess, SigmaGuess, ErfcFracGuess, ID = None):
    ...
    if ID:
        ...
        frame = RotatedEnergy.frame(ROOT.RooFit.Bins(200), ROOT.RooFit.Range(0., 6000.))
...
def GetResolutionForTheta(ID, EnergyList2D, Theta):
    ...
    # RotatedEnergy = (Energy2D[0] * math.cos(Theta) + Energy2D[1] * math.sin(Theta))

    # Find the peak.
    # InitPeakPos = FindPeak(RotatedEnergy)
    print InitPeakPos
    ...
    PeakPos, Sigma, ErfcFrac = DoFit(dataSet, PeakPos[0], Sigma[0], ErfcFrac[0], ID = (ID, '%.5f' % Theta))

    Resolution = DivideWithErrors(Sigma, PeakPos)
    print Resolution, PeakPos
...
def Run(EnergyList2D_ss, EnergyList2D_ms, ThetaToTry, prefix_local):
    ...
    try:
        for TestTheta in ThetaToTry:
            ...
            TestResolution_ss.append(ssResolution)
            TestResolution_ms.append(msResolution)
        ...
        print 'best resSS:\t', BestRes_ss, BestTheta_ss
        print 'best resMS:\t', BestRes_ms, BestTheta_ms
        ...
        return { 'TestTheta': ThetaToTry,
                 'TestResolution_ss': TestResolution_ss,
                 'TestResolution_ms': TestResolution_ms,
                 'Theta_ss' : (ssPolynomialFunc.GetParameter(2), ssPolynomialFunc.GetParError(2)),
                 'BestRes_ss': (ssPolynomialFunc.GetParameter(0), ssPolynomialFunc.GetParError(0)),
                 'Par1_ss': (ssPolynomialFunc.GetParameter(1), ssPolynomialFunc.GetParError(1)),
                 'Theta_ms' : (msPolynomialFunc.GetParameter(2), msPolynomialFunc.GetParError(2)),
                 'BestRes_ms': (msPolynomialFunc.GetParameter(0), msPolynomialFunc.GetParError(0)),
                 'Par1_ms': (msPolynomialFunc.GetParameter(1), msPolynomialFunc.GetParError(1)),
                 'PeakPos_ss': ssMean,
